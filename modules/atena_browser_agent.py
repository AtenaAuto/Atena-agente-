...
class AtenaBrowserAgent:
    ...
    def _get_stealth_context_options(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "timezone_id": random.choice(["America/Sao_Paulo", "America/New_York", "Europe/London"]),
            "permissions": [],
            "extra_http_headers": {
    ...
    async def launch(self, headless: bool = True, proxy: Optional[str] = None):
        ...
        if self.stealth_mode:
            logger.info(f"   Fingerprint: {context_options.get('viewport')}, UA: {context_options.get('user_agent', 'default')[:50]}...")
    ...
    async def navigate(self, url: str, use_cache: bool = True, cache_max_age_hours: int = 24) -> Tuple[bool, Optional[str]]:
        """
    ...
    async def analyze_page(self, content: str, url: str) -> PageAnalysis:
        ...
        keywords = [k.strip() for k in meta_keywords.get('content', '').split(',')] if meta_keywords else []
        ...
        readability_score = min(1.0, max(0.0, (1.0 - (avg_word_length / 15)) * 0.7 + (min(1.0, sentences / max(len(words), 1) * 100) * 0.3)))
    ...
    async def search_and_extract(self, objective: str, base_query: str, max_results: int = 5) -> List[Dict[str, Any]]:
        """
    ...
    def _calculate_relevance(self, analysis: PageAnalysis, objective: str) -> float:
        ...
        title_score = sum(1 for w in objective_words if w in analysis.title.lower()) / max(len(objective_words), 1)
        
        # Score baseado em headings
        heading_text = ' '.join([' '.join(v) for v in analysis.headings.values()]).lower()
        heading_score = sum(1 for w in objective_words if w in heading_text) / max(len(objective_words), 1)
        
        # Score baseado em texto principal
        text_score = sum(1 for w in objective_words if w in analysis.main_text.lower()) / max(len(objective_words), 1)
    ...
    def next_objective_query(self, objective: str, base_query: str) -> str:
        ...
        refinements = [
            "site:github.com", "2026 latest", "official docs", "benchmarks",
            "comparison", "best practices", "tutorial", "guide", "api reference",
            "stackoverflow", "reddit", "example code", "documentation"
        ]
    ...
    def record_search_outcome(self, objective: str, query: str, url: str, usefulness_score: float, notes: str = ""):
        ...
        objective_key = objective.strip().lower()
        stats = self.learning_memory["objective_stats"].setdefault(objective_key, {"iterations": 0, "successful_queries": []})
...
async def main_demo():
    ...
    try:
        ...
        results = await browser_agent.search_and_extract(objective, "ATENA autonomous AI agent architecture", max_results=3)
